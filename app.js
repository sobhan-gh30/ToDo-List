...
newTask.addEventListener("click", function() {
    //changing btn style
    newTask.style.cssText = "width:100% ; border-radius:0 ; height:55px ; transform:translateY(0px); display:flex; justify-content:center; align-items:center";
    ...
    input.addEventListener("keypress" , function (key){
        if (key.key === 'Enter'){
            if (input.value !== ""){
                ...
                newTask.style.cssText = "width:18% ; border-radius:20px ; height:50px ; transform:translateY(-45px);";
            } else {
                newTask.innerHTML = "+ New Task";
                newTask.style.cssText = "width:18% ; border-radius:20px ; height:50px ; transform:translateY(-45px);";
            }
